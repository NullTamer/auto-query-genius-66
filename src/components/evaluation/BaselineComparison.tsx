...
const BaselineComparison: React.FC<BaselineComparisonProps> = ({ results }) => {
  const improvementMetrics = [
    {
      name: "Precision",
      value: parseFloat(((results.overall.precision - results.baseline.precision) * 100).toFixed(2)),
    },
    {
      name: "Recall",
      value: parseFloat(((results.overall.recall - results.baseline.recall) * 100).toFixed(2)),
    },
    {
      name: "F1 Score",
      value: parseFloat(((results.overall.f1Score - results.baseline.f1Score) * 100).toFixed(2)),
    },
  ];
        ...
        <div className="grid grid-cols-3 gap-4 text-center mt-4">
          <div className="p-4 bg-primary/10 rounded-md">
            ...
            <p className="text-xl font-semibold">
              {((results.overall.precision - results.baseline.precision) * 100).toFixed(1)}%
            </p>
          </div>
          <div className="p-4 bg-primary/10 rounded-md">
            <p className="text-sm text-muted-foreground mb-1">Recall Improvement</p>
            <p className="text-xl font-semibold">
              {((results.overall.recall - results.baseline.recall) * 100).toFixed(1)}%
            </p>
          </div>
          <div className="p-4 bg-primary/10 rounded-md">
            <p className="text-sm text-muted-foreground mb-1">F1 Score Improvement</p>
            <p className="text-xl font-semibold">
              {((results.overall.f1Score - results.baseline.f1Score) * 100).toFixed(1)}%
            </p>
          </div>
        </div>
      </Card>
